...
(function ($) {
  ...
  window.createChart = function (data, x, id, title, type, quickLink) {
    ...
    for (var i = 0; i < data.traces.length; i++) {
      ...
      label =
        (data.trace_labels && data.trace_labels[data.traces[i][0]]) ||
        data.traces[i][0].replace(/_/g, " ");
    ...
    if (fixedY) {
      layout.yaxis = { range: [0, fixedYMax] };
    }
      ...
      for (i = 0; i < summaryLabels.length; i++) {
        ...
        if (data.trace_order) {
          percircleOptions.progressBarColor =
            data.colors[data.trace_order.indexOf(key)];
        }
        ...
        container.append(
          $(
            '<div id="' +
              ...
              quickLink.url +
              '" class="button quick-link" id="' +
              id +
              '-inline-wifisession-quick-link" title="' +
              quickLink.title +
              '">' +
              quickLink.label +
              "</a></div>",
    ...
    // add tooltip
    container.find(".svg-container").append('<p class="tooltip"></p>');
