...
class TestDatabaseClient(TestMonitoringMixin, unittest.TestCase):
    ...
    def test_read(self, mock_influxdb_client):
        ...
        self.assertIn(
            '|> filter(fn: (r) => r._field == "field1" or r._field == "field2")', query
        )
    ...
    def test_get_query_with_pdb(self):
        ...
        chart = self._create_chart(metric=metric, configuration="line", test_data=False)
    ...
    def test_object_write(self, mock_influxdb_client):
        ...
        self.assertEqual(
            call_args["record"]["tags"]["content_type"],
            ".".join(om.content_type.natural_key()),
        )
