chart_query = {
    'uptime': {
        'influxdb': (
            ...
            "time >= '{time}' AND time <= '{end_date}' AND content_type = '{content_type}' AND "
    ...
    'wifi_clients': {
        'influxdb': (
            ...
            "WHERE time >= '{time}' AND time <= '{end_date}' AND content_type = '{content_type}' "
    ...
    'general_wifi_clients': {
        'influxdb': (
            "SELECT COUNT(DISTINCT({field_name})) AS wifi_clients FROM {key} "
            "WHERE time >= '{time}' AND time <= '{end_date}'"
    ...
    'general_traffic': {
        'influxdb': (
            ...
            "SUM(rx_bytes) / 1000000000 AS download, "
            "((SUM(tx_bytes) + SUM(rx_bytes)) / 1000000000) AS total FROM {key} "
            "WHERE time >= '{time}' AND time <= '{end_date}' {organization_id} {location_id} "
            "{floorplan_id} {ifname} "
            "GROUP BY time(1d)"
        )
    },
    'memory': {
        'influxdb': (
            ...
            "FROM {key} WHERE time >= '{time}' AND time <= '{end_date}' "
...
default_chart_query = [
    "SELECT {field_name} FROM {key} WHERE time >= '{time}' AND time <= '{end_date}'",
