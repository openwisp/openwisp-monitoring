chart_query = {
    'uptime': {
        'influxdb': (
            ...
            "time >= '{time}' AND time <= '{time}' + {timespan} AND content_type = '{content_type}' AND "
    ...
    'wifi_clients': {
        'influxdb': (
            ...
            "WHERE time >= '{time}' AND time <= '{time}' + {timespan} AND content_type = '{content_type}' "
    ...
    'general_traffic': {
        'influxdb': (
            ...
            "SUM(rx_bytes) / 1000000000 AS download, "
            "((SUM(tx_bytes) + SUM(rx_bytes)) / 1000000000) AS total FROM {key} "
            "WHERE time >= '{time}' AND time <= '{time}' + {timespan} {organization_id} {location_id} "
            "{floorplan_id} {ifname} "
            "GROUP BY time(1d)"
        )
    },
    'memory': {
        'influxdb': (
            ...
            "FROM {key} WHERE time >= '{time}' AND time <= '{time}' + {timespan} "
